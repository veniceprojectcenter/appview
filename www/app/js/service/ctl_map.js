app.init_map = function(ctl, $firebaseArray, NgMap){
  ...
  ctl.showModal = function(event,modal,marker){
    ctl.modal = {};
    //console.log(marker);
    alert("stop");
    ctl.modal.ckid = marker.birth_certificate.ckid;
    ctl.modal.pos = [marker.birth_certificate.lat, marker.birth_certificate.lon];
    //ctl.map.showInfoWindow('foo-iw', marker);
    $(modal).show();
    ctl.apply();
  }
